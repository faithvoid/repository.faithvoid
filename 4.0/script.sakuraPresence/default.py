    ...
    try:
        while True:
            ...
            if data == b'XBDSTATS_ONLINE':
                xbmc.executebuiltin('XBMC.Notification("sakuraPresence", "Server found!", 4000, icon-sakura.png)')
                return addr[0]
...
def main():
    ...
    if not server_ip:
        xbmc.executebuiltin('XBMC.Notification("sakuraPresence", "No sakuraPresence server found! Restart your server/client and try again.", 5000, icon-sakura.png)')
        return
    ...
    # Build payload with only one of the keys
    ...
    if send_to_server(payload, server_ip):
        ...
        xbmc.executebuiltin('XBMC.Notification("sakuraPresence", "Failed to send title ID to server!", 3000, icon-sakura.png)')
